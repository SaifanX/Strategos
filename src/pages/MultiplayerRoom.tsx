          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
              <button 
                ...
                className={`p-10 rounded-2xl border transition-all duration-500 ${choiceMade ? 'opacity-50 border-white/10 cursor-not-allowed' : 'border-blue-500/30 bg-blue-500/5 hover:bg-blue-500/10 hover:border-blue-500 hover:-translate-y-2 hover:shadow-[0_15px_40px_rgba(59,130,246,0.2)]'} flex flex-col items-center gap-6 group`}
              >
                <div className="text-3xl font-bold uppercase tracking-tighter text-blue-400 group-hover:scale-105 transition-transform duration-500">{optionA}</div>
                <div className="text-[10px] font-mono text-blue-400/60 text-center leading-relaxed">{descA}</div>
              </button>
              
              <button 
                disabled={choiceMade}
                onClick={() => handleChoice(optionB)}
                className={`p-10 rounded-2xl border transition-all duration-500 ${choiceMade ? 'opacity-50 border-white/10 cursor-not-allowed' : 'border-jet-orange/30 bg-jet-orange/5 hover:bg-jet-orange/10 hover:border-jet-orange hover:-translate-y-2 hover:shadow-[0_15px_40px_rgba(255,69,0,0.2)]'} flex flex-col items-center gap-6 group`}
              >
                <div className="text-3xl font-bold uppercase tracking-tighter text-jet-orange group-hover:scale-105 transition-transform duration-500">{optionB}</div>
                <div className="text-[10px] font-mono text-jet-orange/60 text-center leading-relaxed">{descB}</div>
              </button>
            </div>
            
            {choiceMade && (
              <div className="text-sm font-mono text-white/40 animate-pulse uppercase mt-8 border border-white/5 bg-white/5 p-4 inline-block">
                  ...
                  <div key={p._id} className={`p-6 border rounded-xl ${isWinner ? 'border-jet-orange bg-jet-orange/10 shadow-[0_0_20px_rgba(255,69,0,0.15)]' : 'border-white/10 bg-white/5'}`}>
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between border-b border-white/5 pb-2">
                        <span className="text-white/40 text-[10px] uppercase">Archetype</span>
                        <span className="text-blue-400 font-bold text-right text-xs">{s.type}</span>
                      </div>
                      <div className="flex justify-between border-b border-white/5 pb-2">
                        <span className="text-white/40 text-[10px] uppercase">{optionA} Bias</span>
                        <span>{Math.round((s.aCount / room.totalRounds)*100)}%</span>
                      </div>
                      <div className="flex justify-between border-b border-white/5 pb-2">
                        <span className="text-white/40 text-[10px] uppercase">{optionB} Bias</span>
